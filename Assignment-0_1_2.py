...
#To get XOR function we need H(b)=1, H(w1+b)=1, H(w2+b)=1 and H(w1+w2+b)=0. This means we need b≤0 and w1+b>0 and w2+b>0 thus w1+w2+2b>0. However we also need w1+w2+b≤0, this means b>0. This is in contradiction with b≤0, so we can not make XOR function this way.
